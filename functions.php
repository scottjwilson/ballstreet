 ...
 */
function ball_street_detect_vite_server() {
    ...
    if (!is_wp_error($response) && wp_remote_retrieve_response_code($response) === 200) {
        ...
        // Try with base path
        $client_response2 = @wp_remote_get($vite_server . '/wp-content/themes/ballstreet/@vite/client', array(
            'timeout' => 1,
            'sslverify' => false,
            'redirection' => 0
        ));
        ...
        if (!is_wp_error($client_response2) && wp_remote_retrieve_response_code($client_response2) === 200) {
            return array('running' => true, 'base' => '/wp-content/themes/ballstreet/', 'server' => $vite_server);
        }
...
function ball_street_enqueue_assets() {
    ...
    $vite = ball_street_detect_vite_server();
 ...
 */
function ball_street_output_vite_scripts() {
    $vite = ball_street_detect_vite_server();
...
add_action('wp_head', 'ball_street_output_vite_scripts', 1);
 ...
 */
function ball_street_script_loader_tag($tag, $handle, $src) {
...
add_filter('script_loader_tag', 'ball_street_script_loader_tag', 10, 3);
